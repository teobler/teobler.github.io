...
(function() {
  ...
  (function init() {
    ...
    if ($asideBar) {
      var asideBarHeight = parseInt(window.getComputedStyle($asideBar)['height'], 10);
      var winHeight = document.documentElement.clientHeight;
      console.log(asideBarHeight, winHeight);
      if (asideBarHeight + 20 > winHeight) {
        return;
      }
      scrollTop > 180 ? Util.addClass($asideBar, 'fixed') : Util.removeClass($asideBar, 'fixed');
      scrollTop > 180 ? Util.addClass($postContainer, 'fixed') : Util.removeClass($postContainer, 'fixed');
    }
  ...
  Util.bind(window, 'scroll', function() {
    ...
    if ($asideBar) {
      var asideBarHeight = parseInt(window.getComputedStyle($toc)['height'], 10);
      var winHeight = document.documentElement.clientHeight;
      if (asideBarHeight + 20 > winHeight) {
          return;
      }
